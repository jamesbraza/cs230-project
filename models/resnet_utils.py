...
def _make_nonshortcut(
    x: KerasTensor,
    filters: Tuple[int, int, int],
    base_name: str,
    first_strides: Tuple[int, int] = (1, 1),
) -> KerasTensor:
    """Build the core identity block's non-shortcut branch."""
    for layer in [
...
def make_identity_block(
    ...
    x_shortcut = x
    x_nonshortcut = _make_nonshortcut(x, filters, base_name)
...
def make_convolutional_block(
    ...
    x_nonshortcut = _make_nonshortcut(x, filters, base_name, first_strides)
    x = tf.keras.layers.Add(name=f"{base_name}_add")([x_shortcut, x_nonshortcut])
    return tf.keras.layers.Activation(activation="relu", name=f"{base_name}_out")(x)
